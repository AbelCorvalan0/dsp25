...
void loadData                 (void);
uint16_t processData(uint16_t sam);
...
q15_t    applyFIRFilter   (q15_t sample);
uint16_t q15_to_uint16     (q15_t q_val);
void     showData_q15             (void);

/* FIR instance and state buffer */
arm_fir_instance_q15 S;
//arm_fir_instance_q15 filter_arrays[][];


#define numTaps 86

///// LP_8K
const q15_t firCoeffsQ15[numTaps] = {
...
int main(void)
{
    ...
    //arm_fir_init_q15(&S, numTaps, firCoeffsQ15, firStateQ15, BUFFER_SIZE);
    arm_fir_init_q15(&S, (sizeof(lp8k)/sizeof(lp8k[0])), lp8k, firStateQ15, 1);
...
q15_t conv16_2_q15    (uint16_t sam){
	//int32_t temp = ((int32_t)sam * 2) - 32768;
	int32_t temp = ((int32_t)sam) - 32768;

	// Limitar al rango Q15 válido
	if (temp < -32768) return -32768;  // -1.0 in Q15
	if (temp > 32767) return 32767;    // ~0.99997 in Q15
...
q15_t applyFIRFilter(q15_t sample){
    q15_t output;
    arm_fir_q15(&S, &sample, &output, 1); // One sample processing.
    outputQ15[0] = output;                // Save processed sample.
    return output;
}

uint16_t q15_to_uint16(q15_t q_val){
    // 1. Saturate the input to Q15 limits
    if (q_val > 32767)
        q_val = 32767;
    else if (q_val < -32768)
        q_val = -32768;
...
void loadData(){
	...
	outputDAC[k]= q15_to_uint16(applyFIRFilter(conv16_2_q15(result.convValue)));
...
void configTimerMatch(){
	...
	CTIMER0_config.input    =                 0; 
	CTIMER0_config.prescale =                 0; 
    ...
    CTIMER0_Match_3_config.matchValue         =                    3000; 
...
void showSamplesInDAC(){
	...
	DAC_SetData(DAC0, outputQ15[indexWaveForm]);
...
void showDataInDAC(){
    ...
    dacValue= outputDAC[indexWaveForm2]; //>>4
