...
uint16_t q15_to_uint16(q15_t q_val){
    ...
    // 2. Shift range [-32768, 32767] → [0, 65535]
    //u16_value = (uint16_t)(((int32_t)q15_value + 32768) & 0xFFFF);
...
void loadData(){
	...
	outputQ15[k]= conv16_2_q15(result.convValue);
	k= (k + 1)%BUFFER_SIZE;
}
...
void showSamplesInDAC(){
	...
	dacValue = q15_to_uint16(conv16_2_q15(waveForm[indexWaveForm]));
...
void showDataInDAC(){
    ...
    dacValue= outputDAC[indexWaveForm2]; 
	DAC_SetData(DAC0, dacValue);
